...
import { useConversations, useMessageSearch } from '@casa/api';
import type { ConversationListItem } from '@casa/api/src/hooks/useConversations';
import type { MessageSearchResult } from '@casa/api/src/hooks/useMessageSearch';
...
function ConversationCard({ item }: { item: ConversationListItem }) {
  const name = item.other_participant?.full_name || item.title || 'Conversation';
...
function SearchResultCard({ item }: { item: MessageSearchResult }) {
  return (
    <TouchableOpacity
      style={styles.conversationCard}
      onPress={() => router.push(`/(app)/messages/${item.conversation_id}` as any)}
      activeOpacity={0.7}
    >
...
export default function OwnerConversationsList() {
  const { conversations, loading, error, refreshing, totalUnread, refreshConversations } = useConversations();
  const { results, loading: searchLoading, hasSearched, search, clearSearch } = useMessageSearch();
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        ...
        <View style={styles.headerActions}>
          <TouchableOpacity style={styles.headerButton} onPress={handleToggleSearch}>
            <Svg width={20} height={20} viewBox="0 0 24 24" fill="none">
              {showSearch ? (
                <Path d="M18 6L6 18M6 6l12 12" stroke={THEME.colors.textPrimary} strokeWidth={1.5} strokeLinecap="round" />
              ) : (
                <>
                  <Circle cx={11} cy={11} r={8} stroke={THEME.colors.textPrimary} strokeWidth={1.5} />
                  <Path d="M21 21l-4.35-4.35" stroke={THEME.colors.textPrimary} strokeWidth={1.5} strokeLinecap="round" />
                </>
              )}
            </Svg>
          </TouchableOpacity>
      ...
      {showSearch && (
        <View style={styles.searchBar}>
          <Svg width={16} height={16} viewBox="0 0 24 24" fill="none">
            <Circle cx={11} cy={11} r={8} stroke={THEME.colors.textTertiary} strokeWidth={1.5} />
            <Path d="M21 21l-4.35-4.35" stroke={THEME.colors.textTertiary} strokeWidth={1.5} strokeLinecap="round" />
          </Svg>
          <TextInput
          ...
          />
          {searchLoading && <ActivityIndicator size="small" color={THEME.colors.brand} />}
        </View>
      )}
      ...
      {showSearch && hasSearched ? (
        ...
        <View style={styles.centered}>
          ...
          <Text style={styles.emptySubtitle}>
            Start a conversation with a tenant to get started.
          </Text>
          <TouchableOpacity
