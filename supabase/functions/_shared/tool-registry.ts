...
export const CLAUDE_TOOLS: AgentToolDefinition[] = [
  ...
  { name: 'send_email_sendgrid', description: 'Send email via Resend. Supports CC/BCC for copying owners and tenants on maintenance and trade correspondence. Pass maintenance_request_id to log the email in the maintenance audit trail.', input_schema: { type: 'object', properties: { to: { type: 'string', description: 'Primary recipient email' }, subject: { type: 'string' }, html_content: { type: 'string', description: 'Email body HTML' }, cc: { type: 'array', items: { type: 'string' }, description: 'CC recipients (e.g. owner on trade emails)' }, bcc: { type: 'array', items: { type: 'string' }, description: 'BCC recipients (e.g. landlord on tenant emails)' }, reply_to: { type: 'string', description: 'Reply-to email address' }, maintenance_request_id: { type: 'string', description: 'Link email to maintenance request for audit trail' } }, required: ['to', 'subject', 'html_content'] } },
