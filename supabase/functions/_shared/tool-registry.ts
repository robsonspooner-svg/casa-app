...
export const CLAUDE_TOOLS: AgentToolDefinition[] = [
  ...
  { name: 'estimate_cost', description: 'Estimate maintenance cost using platform-wide data from completed jobs + market rates', input_schema: { type: 'object', properties: { category: { type: 'string', description: 'Trade category (plumbing, electrical, appliance, hvac, structural, pest, locks_security, garden_outdoor, cleaning, other)' }, description: { type: 'string', description: 'Work description' }, postcode: { type: 'string', description: 'Property postcode for location-based pricing' }, state: { type: 'string', description: 'Property state (NSW, VIC, QLD, SA, WA, TAS, NT, ACT) for regional pricing' } }, required: ['category', 'description'] } },
  ...
  { name: 'update_document', description: 'Update an existing document content and/or metadata. Use when the owner requests revisions (e.g. "change the rent to $620/week"). The previous version is preserved in document_versions automatically.', input_schema: { type: 'object', properties: { document_id: { type: 'string', description: 'Document UUID to update' }, html_content: { type: 'string', description: 'New full HTML content' }, title: { type: 'string', description: 'New title (optional)' }, tags: { type: 'array', items: { type: 'string' }, description: 'Updated tags' } }, required: ['document_id', 'html_content'] } },
  ...
  { name: 'record_quote_response', description: 'Record a quote response from a tradesperson on a work order. Use when the owner reports a quote amount received from a trade.', input_schema: { type: 'object', properties: { work_order_id: { type: 'string', description: 'Work order UUID' }, quoted_amount: { type: 'number', description: 'Quoted amount in AUD (inc. GST)' }, availability: { type: 'string', description: 'Trade availability/proposed date' }, notes: { type: 'string', description: 'Additional notes from the trade' } }, required: ['work_order_id', 'quoted_amount'] } },
  { name: 'generate_work_order', description: 'Generate a professional work order document for a trade, save it to documents, and optionally email it to the tradesperson with the owner CC\'d.', input_schema: { type: 'object', properties: { work_order_id: { type: 'string', description: 'Work order UUID to generate document for' }, send_to_trade: { type: 'boolean', description: 'Email the work order to the assigned trade (default: true)' } }, required: ['work_order_id'] } },

  // ── ADDITIONAL QUERY/ACTION TOOLS ──────────────────────────────────────
  { name: 'add_manual_expense', description: 'Add a manual expense for a property (council rates, insurance, strata, repairs, etc.). Creates an expense record for financial reporting and tax purposes.', input_schema: { type: 'object', properties: { property_id: { type: 'string', description: 'Property UUID' }, description: { type: 'string', description: 'Expense description' }, amount: { type: 'number', description: 'Amount in AUD' }, expense_date: { type: 'string', description: 'Expense date (ISO 8601, defaults to today)' }, tax_category: { type: 'string', enum: ['insurance', 'council_rates', 'strata', 'repairs', 'interest', 'depreciation', 'management_fees', 'legal', 'advertising', 'travel', 'other'], description: 'Tax category for ATO reporting' }, is_tax_deductible: { type: 'boolean', description: 'Whether tax deductible (default true)' }, is_recurring: { type: 'boolean', description: 'Whether this is a recurring expense' }, recurring_frequency: { type: 'string', enum: ['weekly', 'fortnightly', 'monthly', 'quarterly', 'yearly'], description: 'Recurring frequency' }, notes: { type: 'string', description: 'Additional notes' } }, required: ['property_id', 'description', 'amount'] } },
  { name: 'analyze_payment_patterns', description: 'Analyse payment patterns for a tenancy — identifies late payment trends, average days late, payment consistency score, and risk assessment', input_schema: { type: 'object', properties: { tenancy_id: { type: 'string', description: 'Tenancy UUID' }, months: { type: 'number', description: 'Number of months to analyse (default 12)' } }, required: ['tenancy_id'] } },
  { name: 'get_document_access_log', description: 'Get access log for a document — who viewed, downloaded, printed, or shared it, with timestamps', input_schema: { type: 'object', properties: { document_id: { type: 'string', description: 'Document UUID' }, limit: { type: 'number', description: 'Max entries to return (default 50)' } }, required: ['document_id'] } },
  { name: 'remind_unsigned_documents', description: 'Send reminders for documents that need signing. Finds pending documents and sends notifications to the relevant parties.', input_schema: { type: 'object', properties: { property_id: { type: 'string', description: 'Filter by property (optional)' }, tenancy_id: { type: 'string', description: 'Filter by tenancy (optional)' }, document_id: { type: 'string', description: 'Specific document to remind about (optional)' } } } },
  ...
  { name: 'trade_negotiation_email', description: 'Send a negotiation/quote/work email to a tradesperson using a Casa persona. Use this for all trade correspondence — requesting quotes, negotiating prices, sending work orders, and follow-ups. The agent writes the email body following negotiation principles (get good prices, be skeptical, leverage volume). Automatically uses a consistent persona name and CCs the owner.', input_schema: { type: 'object', properties: { trade_id: { type: 'string', description: 'Trade UUID from the owner network' }, email_type: { type: 'string', enum: ['quote_request', 'negotiation', 'followup', 'work_order'], description: 'Type of trade email' }, job_title: { type: 'string', description: 'Short description of the job (e.g. "Leaking tap repair")' }, property_address: { type: 'string', description: 'Property address for the job' }, email_body: { type: 'string', description: 'HTML email body (the agent composes this using negotiation principles)' }, maintenance_request_id: { type: 'string', description: 'Link to maintenance request for audit trail' }, work_order_id: { type: 'string', description: 'Link to work order if applicable' }, cc_owner: { type: 'boolean', description: 'CC the owner on this email (default: true)' } }, required: ['trade_id', 'email_type', 'job_title', 'property_address', 'email_body'] } },
  { name: 'send_email_resend', description: 'Send email via Resend with mandatory context_type for security. Every email must declare its purpose (e.g. "rent_reminder", "owner_to_tenant", "trade_quote_request"). System notifications use noreply@casaapp.com.au. Trade emails automatically use a persona. Recipient must be in the user\'s network (tenant or trade). For trade emails, prefer trade_negotiation_email tool instead.', input_schema: { type: 'object', properties: { to: { type: 'string', description: 'Primary recipient email' }, subject: { type: 'string' }, html_content: { type: 'string', description: 'Email body HTML' }, context_type: { type: 'string', enum: ['rent_reminder', 'arrears_notice', 'payment_received', 'payment_receipt', 'connection_code', 'tenant_welcome', 'document_shared', 'lease_for_signature', 'inspection_notice', 'maintenance_update', 'compliance_reminder', 'lease_expiry_notice', 'rent_increase_notice', 'application_update', 'breach_notice', 'trade_quote_request', 'trade_negotiation', 'trade_work_order', 'trade_scheduling', 'trade_followup', 'owner_to_tenant', 'owner_to_trade'], description: 'REQUIRED: The purpose of this email. Determines sender identity and security validation.' }, cc: { type: 'array', items: { type: 'string' }, description: 'CC recipients' }, bcc: { type: 'array', items: { type: 'string' }, description: 'BCC recipients' }, reply_to: { type: 'string', description: 'Reply-to email address' }, maintenance_request_id: { type: 'string', description: 'Link to maintenance request for audit trail' } }, required: ['to', 'subject', 'html_content', 'context_type'] } },
...
export const TOOL_META: Record<string, ToolMeta> = {
  ...
  update_document: { category: 'action', autonomyLevel: 2, riskLevel: 'low', reversible: true },
  ...
  record_quote_response: { category: 'action', autonomyLevel: 2, riskLevel: 'low', reversible: true },
  generate_work_order: { category: 'generate', autonomyLevel: 2, riskLevel: 'medium', reversible: false },
  add_manual_expense: { category: 'action', autonomyLevel: 2, riskLevel: 'low', reversible: true },
  analyze_payment_patterns: { category: 'query', autonomyLevel: 4, riskLevel: 'none', reversible: false },
  get_document_access_log: { category: 'query', autonomyLevel: 4, riskLevel: 'none', reversible: false },
  remind_unsigned_documents: { category: 'action', autonomyLevel: 2, riskLevel: 'low', reversible: false },
  ...
  send_email_sendgrid: { category: 'integration', autonomyLevel: 2, riskLevel: 'low', reversible: false }, // legacy alias
  send_email_resend: { category: 'integration', autonomyLevel: 2, riskLevel: 'low', reversible: false },
  trade_negotiation_email: { category: 'integration', autonomyLevel: 2, riskLevel: 'low', reversible: false },
...
const TOOL_GROUPS: Record<string, string[]> = {
  ...
  communication: [
    ...
    'send_sms_twilio', 'send_email_sendgrid', 'send_email_resend', 'send_push_expo',
    'trade_negotiation_email',
